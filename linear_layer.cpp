...
LinearLayer::LinearLayer(Index in_size, Index out_size,
...
Matrix LinearLayer::Forward(const Matrix &x) {
  ...
  a_ = activation_function_.Activate(z_);
  return a_;
}

Matrix LinearLayer::Backward(const Matrix &u, double learn_rate) {
  assert(learn_rate >= 0);
  assert(u.rows() == A_.rows());
  assert(u.cols() == x_.cols());

  Matrix dz = u.array() * activation_function_.Derivative(z_).array();

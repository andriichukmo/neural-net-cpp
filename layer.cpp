...
Layer::Layer(In in_size, Out out_size,
...
Matrix Layer::Backward(const Matrix &grad, double learn_rate) {
  ...
  Matrix dz;
  if (auto softmax =
          dynamic_cast<SoftMaxActivation *>(activation_function_.get())) {
    dz = grad;
  } else {
    dz = grad.array() * activation_function_->Differential(cache_->y).array();
  }
